...
class OT2GymEnv(gym.Env):
    ...
    def _compute_reward(self, current_pos, target_pos, action, distance, previous_distance):
        ...
        action_penalty = -0.5 * action_magnitude  # More significant than before
        ...
        if distance < 0.01 and self.current_step % 10 == 0:  # Close to target
            print(f"Distance: {distance*1000:.2f}mm, Total reward: {total_reward:.2f}")
            print(f"  Coarse: {coarse_reward:.1f}, Fine: {fine_reward:.1f}")
            print(f"  Precision: {precision_reward:.1f}, Progress: {progress_reward:.1f}")
            print(f"  Goal: {goal_reward:.1f}")
