...
Usage:
    python train_rl.py --algorithm PPO --total_timesteps 2000000 --n_envs 1 \
        --learning_rate 3e-4 --run_name "my_experiment"
"""
    ...
    def _on_step(self) -> bool:
        ...
        if success_rate > self.best_success_rate or \
           (success_rate == self.best_success_rate and mean_reward > self.best_mean_reward):
            ...
            if self.verbose > 0:
                print(f"🏆 New best model! Success: {success_rate*100:.1f}%, Saved to: {self.save_path}.zip\n")
...
def train(args):
    ...
    # ===================== W&B Initialization =====================
    # W&B will auto-login using WANDB_API_KEY environment variable
    config = {
    ...
    eval_callback = DetailedEvalCallback(
        ...
        n_eval_episodes=10,
        save_path=f"{args.save_dir}/best/best_model",
        verbose=1
    )
    ...
    print(f"Expected time: ~{args.total_timesteps / 35 / 3600:.1f} hours")
    print("="*80 + "\n")
    ...
    try:
        ...
        # Save final model
        final_path = f"{args.save_dir}/{args.algorithm}_{args.run_name}_final"
        ...
        best_path = f"{args.save_dir}/best/best_model.zip"
        if os.path.exists(best_path):
            task.upload_artifact('best_model', best_path)
            print(f"✓ Best model: {best_path}")
        
        print(f"✓ Models uploaded to ClearML artifacts")
